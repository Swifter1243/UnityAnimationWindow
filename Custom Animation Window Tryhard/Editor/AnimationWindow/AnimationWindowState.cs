...
namespace UnityEditorInternal.Enemeteen {
	...
	internal class AnimationWindowState : ScriptableObject, ICurveEditorState {
		...
		private UnityEditor.EditorCurveBinding? m_lastAddedCurveBinding;
		...
		private void Refresh() {
			...
			if (refresh == RefreshType.Everything) {
				...
				if (m_lastAddedCurveBinding != null)
					OnNewCurveAdded((UnityEditor.EditorCurveBinding) m_lastAddedCurveBinding);
		...
		public void OnEnable() {
			...
			UnityEditor.AnimationUtility.onCurveWasModified += CurveWasModified;
		...
		void CurveWasModified(AnimationClip clip, UnityEditor.EditorCurveBinding binding, UnityEditor.AnimationUtility.CurveModifiedType type) {
			...
			if (type == UnityEditor.AnimationUtility.CurveModifiedType.CurveModified) {
				bool didFind = false;
		...
		public void SaveKeySelection(string undoLabel) {
			if (m_KeySelection != null)
				UnityEditor.Undo.RegisterCompleteObjectUndo(m_KeySelection, undoLabel);
		}
		...
		public void SaveCurve(AnimationClip clip, AnimationWindowCurve curve, string undoLabel) {
			...
			UnityEditor.Undo.RegisterCompleteObjectUndo(clip, undoLabel);
		...
		public void RemoveCurve(AnimationWindowCurve curve, string undoLabel) {
			...
			UnityEditor.Undo.RegisterCompleteObjectUndo(curve.clip, undoLabel);

			if (curve.isPPtrCurve)
				UnityEditor.AnimationUtility.SetObjectReferenceCurve(curve.clip, curve.binding, null);
			else
				UnityEditor.AnimationUtility.SetEditorCurve(curve.clip, curve.binding, null);
		}
		...
		private void UpdateSelectionFilter() {
			m_SelectionFilter = (filterBySelection) ? (int[]) UnityEditor.Selection.instanceIDs.Clone() : null;
		}
		...
		private void OnNewCurveAdded(UnityEditor.EditorCurveBinding newCurve) {
		...
		public float clipFrameRate {
			...
			set {
				...
				if (activeAnimationClip != null && value > 0 && value <= 10000) {
					...
					AnimationEvent[] events = UnityEditor.AnimationUtility.GetAnimationEvents(activeAnimationClip);
					...
					UnityEditor.AnimationUtility.SetAnimationEvents(activeAnimationClip, events);
		...
		public int currentFrame { get { return time.frame; } set { controlInterface.SetCurrentFrame(value); } }
		public float currentTime { get { return time.time; } set { controlInterface.SetCurrentTime(value); } }
