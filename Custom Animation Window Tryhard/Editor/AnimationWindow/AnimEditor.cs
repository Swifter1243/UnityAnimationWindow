...
namespace UnityEditor.Enemeteen {
	...
	internal class AnimEditor : ScriptableObject {
		...
		internal static PrefColor kEulerXColor = new PrefColor("Enemeteen/EulerX", 1.0f, 0.0f, 1.0f, 1.0f);
		internal static PrefColor kEulerYColor = new PrefColor("Enemeteen/EulerY", 1.0f, 1.0f, 0.0f, 1.0f);
		internal static PrefColor kEulerZColor = new PrefColor("Enemeteen/EulerZ", 0.0f, 1.0f, 1.0f, 1.0f);
		...
		public AnimationWindowControl controlInterface {
			get {
				return state.controlInterface;
			}
		}

		public AnimationWindowControl overrideControlInterface {
			get {
		...
		private void TabSelectionOnGUI()
		{
			...
			//GUILayout.Label("play range end");
		...
		private void PlayControlsOnGUI() {
			...
			if (EditorGUI.EndChangeCheck()) {
				controlInterface.SetCurrentFrame(newFrame);
			}
		}
		...
		static void ExecuteShortcut(ShortcutArguments args, Action<AnimEditor> exp) {
			var animationWindow = (EnemeteenWindow) args.context;
		...
		static void ExecuteShortcut(ShortcutArguments args, Action<AnimationWindowControl> exp) {
			ExecuteShortcut(args, animEditor => exp(animEditor.controlInterface));
		}

		[Shortcut("Enemeteen/Show Curves", typeof(EnemeteenWindow), KeyCode.C)]
		static void ShowCurves(ShortcutArguments args) {
			ExecuteShortcut(args, animEditor => { animEditor.SwitchBetweenCurvesAndDopesheet(); });
		}

		[Shortcut("Enemeteen/Play Animation", typeof(EnemeteenWindow), KeyCode.Space)]
		...
		[Shortcut("Enemeteen/Next Frame", typeof(EnemeteenWindow), KeyCode.Period)]
		static void NextFrame(ShortcutArguments args) {
			ExecuteShortcut(args, controlInterface => controlInterface.GoToNextFrame());
		}

		[Shortcut("Enemeteen/Previous Frame", typeof(EnemeteenWindow), KeyCode.Comma)]
		static void PreviousFrame(ShortcutArguments args) {
			ExecuteShortcut(args, controlInterface => controlInterface.GoToPreviousFrame());
		}

		[Shortcut("Enemeteen/Previous Keyframe", typeof(EnemeteenWindow), KeyCode.Comma, ShortcutModifiers.Alt)]
		...
		[Shortcut("Enemeteen/Next Keyframe", typeof(EnemeteenWindow), KeyCode.Period, ShortcutModifiers.Alt)]
		static void NextKeyFrame(ShortcutArguments args) {
			ExecuteShortcut(args, controlInterface => controlInterface.GoToNextKeyframe());
		}

		[Shortcut("Enemeteen/First Keyframe", typeof(EnemeteenWindow), KeyCode.Comma, ShortcutModifiers.Shift)]
		static void FirstKeyFrame(ShortcutArguments args) {
			ExecuteShortcut(args, controlInterface => controlInterface.GoToFirstKeyframe());
		}

		[Shortcut("Enemeteen/Last Keyframe", typeof(EnemeteenWindow), KeyCode.Period, ShortcutModifiers.Shift)]
		static void LastKeyFrame(ShortcutArguments args) {
			ExecuteShortcut(args, controlInterface => controlInterface.GoToLastKeyframe());
		}

		[Shortcut("Enemeteen/Key Selected", null, KeyCode.K)]
		static void KeySelected(ShortcutArguments args) {
			EnemeteenWindow animationWindow = EnemeteenWindow.GetAllAnimationWindows().Find(aw => (aw.state.previewing || aw == EditorWindow.focusedWindow));
		...
		[Shortcut("Enemeteen/Key Modified", null, KeyCode.K, ShortcutModifiers.Shift)]
		...
		[Shortcut("Enemeteen/Toggle Ripple", typeof(EnemeteenWindow), KeyCode.Alpha2, ShortcutModifiers.Shift)]
		static void ToggleRipple(ShortcutArguments args) {
			ExecuteShortcut(args, (Action<AnimEditor>) (animEditor => { animEditor.state.rippleTime = !animEditor.state.rippleTime; }));
		}

		[ClutchShortcut("Enemeteen/Ripple (Clutch)", typeof(EnemeteenWindow), KeyCode.Alpha2)]
		static void ClutchRipple(ShortcutArguments args) {
			ExecuteShortcut(args, (Action<AnimEditor>) (animEditor => { animEditor.state.rippleTimeClutch = args.stage == ShortcutStage.Begin; }));
		}

		[Shortcut("Enemeteen/Frame All", typeof(EnemeteenWindow), KeyCode.A)]
		static void FrameAll(ShortcutArguments args) {
			ExecuteShortcut(args, animEditor => { animEditor.triggerFraming = true; });
		}

		[Shortcut("Enemeteen/Toggle Loop", typeof(EnemeteenWindow), KeyCode.L)]
		static void ToggleLoop(ShortcutArguments args) {
			ExecuteShortcut(args, (Action<AnimEditor>) (animEditor => { animEditor.controlInterface.loop = !animEditor.controlInterface.loop; }));
		}

		[Shortcut("Enemeteen/Toggle Play from the beginning", typeof(EnemeteenWindow), KeyCode.B)]
		static void TogglePlayFromBeginning(ShortcutArguments args) {
			ExecuteShortcut(args, (Action<AnimEditor>) (animEditor => { animEditor.controlInterface.playFromBeginning = !animEditor.controlInterface.playFromBeginning; }));
		}
